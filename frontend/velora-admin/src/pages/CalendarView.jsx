      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-7 gap-2 text-xs">
            {Array.from({ length: 31 }).map((_, i) => (
              <div
                key={i}
                className="h-16 p-1.5 rounded-xl border bg-slate-50 border-slate-200 text-slate-700 flex flex-col justify-between hover:border-slate-300"
              >
                <span>{i + 1}</span>
              </div>
            ))}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-4">
          ...
          <div className="space-y-3">
            {events.length === 0 ? (
              <p className="text-slate-400 text-xs text-center py-6">No operational events scheduled today</p>
            ) : (
              events.map((evt, idx) => (
                <div key={idx} className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1.5 text-xs">
                  ...
                  <div className="flex items-center gap-3 text-slate-500 text-[11px]">
                    <span className="flex items-center gap-1">
                      <Clock size={12} /> {evt.time || "Scheduled"}
                    </span>
                    {evt.location && (
                      <span className="flex items-center gap-1">
